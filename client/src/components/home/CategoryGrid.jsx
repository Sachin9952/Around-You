    ...
    <div className="px-4 py-4">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-y-6 gap-x-2">
        {items.map((item, index) => (
          ...
          >
            <div className="relative w-20 h-20 md:w-24 md:h-24 mb-3 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center group-hover:bg-[#F3F1FF] group-hover:border-[#6D5AE6]/30 transition-colors shadow-sm">
              {item.isNew && (
              ...
              {item.icon ? (
                 <item.icon className={`w-10 h-10 md:w-12 md:h-12 ${item.iconColor || 'text-gray-700'} group-hover:scale-110 transition-transform`} />
              ) : item.img ? (
                 <img src={item.img} alt={item.name} className="w-12 h-12 md:w-14 md:h-14 object-contain group-hover:scale-110 transition-transform" />
              ) : null}
            </div>
